...
public class MainWindow {
    ...
    public void updateStatus(WeatherStatus ws) {
        ...
        String statusStr = "<html><div style='text-align: center;'>" + "Showing weather in:<br><b>"
                + ws.getWeather().getCity() + "</b>" + "<br>(" + ws.getWeather().getRegion() + ", "
                + ws.getWeather().getCountry() + ")</div></html>";
        statusLabel_.setText(statusStr);
    }

    public void updateWeather(Weather weather) {
        String weatherStr = "<html><div style='text-align: center;'>" + "Temperature: "
                + weather.getTemperature() + " " + "\u00B0" + "C<br>" + "Wind speed: "
                + weather.getWindSpeed() + " km/h<br>" + "Description: " + weather.getDescription()
                + "<br>" + "Local time: " + weather.getLocalTime() + "</div></html>";
    ...
    private void initWindow() {
        ...
        // Bounds
        searchInput_.setBounds(MID - (searchInputSize.width / 2), 10, searchInputSize.width,
                searchInputSize.height);
        searchButton_.setBounds(MID - (searchButtonSize.width / 2), CELL_HEIGHT + GAP,
                searchButtonSize.width, searchButtonSize.height);
        statusLabel_.setBounds(MID - (statusLabelSize.width / 2), 2 * (CELL_HEIGHT + GAP),
                statusLabelSize.width, statusLabelSize.height);
        weatherLabel_.setBounds(MID - (weatherLabelSize.width / 2), 3 * (CELL_HEIGHT + GAP),
                weatherLabelSize.width, weatherLabelSize.height);
        loadingLabel_.setBounds(MID - (loadingSize.width / 2), 3 * (CELL_HEIGHT + GAP) + 20,
